import { getSearchImagesConfig } from '../../services/designer/designerTabs.service';
import { ITabApi, ITabbarListElement } from '../interfaces/interfaces';

export default async function getSearchImages(
  urls: ITabApi,
  productId: number,
  page: number,
  count: number,
  ...
  const response = await getSearchImagesConfig(urls, productId, page, count, value);
